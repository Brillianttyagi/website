...
def get_file_path_company_gst_doc(instance, filename):
    ext = filename.split('.')[-1]
    filename = "%s.%s" % (uuid4(), ext)
    return join('GSTdoc/', filename)
...
class Account(AbstractUser):
    ...
    companyGSTNumber = models.CharField(max_length=30,null=True,blank=True)
    GSTDoc = models.FileField(storage=KYCDocStorage,upload_to=get_file_path_company_gst_doc,null=True,blank=True)
    gstin_data = models.CharField(max_length=500,null=True,blank=True)
    gstin_data_status = models.CharField(max_length=5,null=True,blank=True)
    # isCompanyGSTNumberVarified = models.BooleanField(default=False)
